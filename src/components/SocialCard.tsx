  ...
  return (
    <div
      className={ClassNames(
        'flex h-full w-64 flex-row rounded-md bg-[#1c1e2b] text-[#f7f8f8] transition-[transform,opacity] duration-200 md:w-96',
        className,
        isOpen
      ...
      ref={CardRef}
    >
      <div className="relative z-10 mt-auto mb-auto flex flex-col p-8  md:w-1/2">
        <div className="my-2 flex w-8 flex-row items-center justify-center rounded-md border  ">
          <Logo classname="h-7 w-7" />
        </div>
        <h1 className="text-xs font-bold md:text-sm">Découvrez le portfolio de {title} </h1>
        <h1 className="text-xs">Parcourez son travail et contactez-le pour vos projets</h1>
      </div>
      <div className="flex shrink-0 flex-col  rounded-r-md bg-[#21232f] md:w-1/2">
        <div className=" mx-auto my-auto flex flex-col items-center">
          {/* es-lint-disable-next-line @next/no-img-element */}
          <Image
            src={image}
            className=" flex h-10 w-10 rounded-full md:h-20 md:w-20 "
            width={200}
            height={200}
            alt="rien du tout "
          />
          <h1 className="text-xs font-bold">@{title}</h1>
          <h2 className="text-xs md:font-medium">{fonction}</h2>
        </div>
        {/**
